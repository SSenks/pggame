...
def check_collision_enemies(object, enemies_list):
    ...
    for enemy in enemies_list:
        #при коллизии объект пропадает из всех групп спрайтов и игра заканчивается
        if object.rect.colliderect(enemy.rect) and abs(player.y_velocity) == 0 and object.rect.bottom != enemy.rect.top:
            # объект пропадает из всех групп спрайтов и игра заканчивается
            object.kill()
            running = False
        elif object.rect.colliderect(enemy.rect) and abs(player.y_velocity)>0 :
            enemy.kill()
...
# создаем текстовое поле для отображения паузы
font = pygame.font.SysFont(None, 40)
paused_text = font.render('Paused. Press ESCAPE to resume', True, WHITE)
